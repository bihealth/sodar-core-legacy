...
class RemoteProjectAPI:
        ...
        try:
            ...
            for k, v in u_data.items():
                if (k not in ['groups', 'sodar_uuid'] and
                        hasattr(user, k) and str(getattr(user, k)) != str(v)):
    ...
    def _handle_project_error(self, error_msg, uuid, p, action):
        ...
        logger.error('{} {} "{}" ({}): {}'.format(
            action.capitalize(), p['type'].lower(), p['title'], uuid,
            error_msg))
    ...
    def _update_project(self, project, p_data, parent):
        ...
        for k, v in p_data.items():
            if (k not in ['parent', 'sodar_uuid', 'roles', 'readme'] and
                    hasattr(project, k) and str(getattr(project, k)) != str(v)):
                updated_fields.append(k)
    ...
    def _update_roles(self, project, p_data):
        ...
        for r_uuid, r in {
                k: v for k, v in p_data['roles'].items() if
                '@' in v['user'] or v['role'] == PROJECT_ROLE_OWNER}.items():
            ...
            if r['role'] == PROJECT_ROLE_OWNER and '@' not in r['user']:
                ...
                self.remote_data['projects'][uuid]['roles'][
                    r_uuid]['user'] = self.default_owner.username
                self.remote_data['projects'][uuid]['roles'][
            ...
            if old_as and r['role'] == PROJECT_ROLE_OWNER:
                ...
                if (('@' in r['user'] and old_as.user != role_user) or
                        (role_user == self.default_owner and
                         project.get_owner().user != self.default_owner)):
                    as_updated = True
    ...
    def _sync_project(self, uuid, p_data):
        ...
        if (p_data['parent_uuid'] and
                p_data['parent_uuid'] not in self.updated_parents):
        ...
        project = Project.objects.filter(
            type=p_data['type'], sodar_uuid=uuid).first()
        parent = None
    ...
    def sync_source_data(self, site, remote_data, request=None):
        ...
        if not {k: v for k, v in self.remote_data['projects'].items() if
        ...
        for sodar_uuid, u_data in {
                k: v for k, v in self.remote_data['users'].items() if
                '@' in v['username']}.items():
        ...
        for sodar_uuid, p_data in {
                k: v for k, v in self.remote_data['projects'].items() if
                v['type'] == PROJECT_TYPE_PROJECT and
